...
summary: Overview of the Fanoni Imaging's scope as a web-based medical imaging viewer that functions as a "dumb client," explaining its capabilities as a Progressive Web Application (PWA), the separation of data from the viewer, and clarification about offline capabilities.
---
# Scope of Project

The Fanoni Imaging is a web based medical imaging viewer. This allows it to be used
on almost any device, anywhere. The Fanoni Imaging is what is commonly referred to
...
To be more specific, the Fanoni Imaging is a collection of HTML, JS, and CSS files.
...
information, the Fanoni Imaging's scope **DOES NOT** encompass providing _any_
data; only the configuration necessary to interface with one or more of these
many data sources. The Fanoni Imaging's scope **DOES** include configuration and
...
the Fanoni Imaging as a desktop application.

_Does the Fanoni Imaging work with the local filesystem?_
...
package the Fanoni Imaging as an [Electron app][electron].
